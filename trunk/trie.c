...
trie_retrieve(const Trie *trie, const AlphaChar *key, TrieData *o_data) {
    ...
    for (p = key; !trie_da_is_separate(trie->da, s); p++) {
        if (!da_walk(trie->da, &s, alpha_map_char_to_trie(trie->alpha_map, *p))) {
            return FALSE;
        }
        if (0 == *p) {
            break;
    ...
    suffix_idx = 0;
    for (;; p++) {
        if (!tail_walk_char(trie->tail, s, &suffix_idx, alpha_map_char_to_trie(trie->alpha_map, *p))) {
            return FALSE;
        }
        if (0 == *p) {
            break;
...
Bool
trie_get_state_data_by_key(const Trie *trie, const AlphaChar *key, TrieData *o_data) {
    TrieIndex s;
    short suffix_idx;
    const AlphaChar *p;
    ...
    s = da_get_root(trie->da);
    for (p = key; !trie_da_is_separate(trie->da, s); p++) {
        //TrieChar * xx =  tail_get_suffix(trie->tail, s);*xx;
        if (!da_walk(trie->da, &s, alpha_map_char_to_trie(trie->alpha_map, *p))) {
            //break;
        }
...
static Bool
trie_store_conditionally(Trie *trie, const AlphaChar *key, TrieData data, Bool is_overwrite) {
    TrieIndex s, t;
    short suffix_idx;
    ...
    suffix_idx = 0;
    for (;; p++) {
        if (!tail_walk_char(trie->tail, t, &suffix_idx, alpha_map_char_to_trie(trie->alpha_map, *p))) {
            TrieChar *tail_str;
            Bool res;
...
static Bool
trie_branch_in_branch(Trie *trie, TrieIndex sep_node, const TrieChar *suffix, TrieData data) {
...
static Bool
trie_branch_in_tail(Trie *trie, TrieIndex sep_node, const TrieChar *suffix, TrieData data) {
...
static TrieState *
trie_state_new(const Trie *trie, TrieIndex index, short suffix_idx, short is_suffix) {
    TrieState *s;
...
trie_state_is_walkable(const TrieState *s, AlphaChar c) {
    ...
    if (!s->is_suffix)
        ...
        return tail_is_walkable_char(s->trie->tail, s->index, s->suffix_idx, tc);
}
...
/**
 * @brief Get data from state
 *
 * @param s    : a state
 *
 * @return the data associated with the state @a s
 *
 * Get value from a state of trie.
 */
TrieData
trie_from_state_get_data(const TrieState *s) {
